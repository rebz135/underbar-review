(function() {
  ...
  _.reject = function(collection, test) {
    ...
    let pass = _.filter(collection, test);
    let result = _.filter(collection, function (val) {
      return _.indexOf(pass, val) >= 0 ? false : true;
    });
    return result;
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    let accum = collection[keys[0]];
    let array;
    if (Array.isArray(collection)) {
      array = collection.slice(1);
    } else {
      array = Object.values(collection).slice(1);
    }
